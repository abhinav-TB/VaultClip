...
export const GemmaChat = ({ settings }: { settings: GenerationSettings }) => {
  ...
  return (
    <div className="flex min-h-[760px] w-full flex-col overflow-hidden rounded-lg border border-slate-800 bg-slate-900 shadow-2xl lg:h-full lg:min-h-0">
      <div className="bg-slate-800/50 p-4 border-b border-slate-800 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          <div className={`w-2 h-2 rounded-full ${
            modelReady ? 'bg-green-500' : modelLoading ? 'bg-yellow-400 animate-pulse' : modelStatus === 'error' ? 'bg-red-500' : 'bg-slate-500'
          }`} />
          <span className="text-sm font-medium text-slate-300">Gemma-E2B Local AI</span>
        </div>
        {modelReady ? (
          ...
          <span className="text-xs text-slate-500 uppercase tracking-widest font-bold">Not loaded</span>
        )}
      </div>

      {!modelReady && (
        <div className="border-b border-slate-800 bg-slate-950/80 px-5 py-4 text-sm">
              ...
              {modelLoading && (
                <div className="space-y-3 rounded-lg border border-slate-800 bg-slate-900/60 p-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="w-10 text-right font-mono text-xs text-slate-400">{modelProgress}%</span>
                  </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4 lg:min-h-0 lg:p-6">
        {messages.length === 0 && (
          <div className="flex min-h-full flex-col items-center justify-center gap-2 text-center text-slate-500">
            ...
            <p className="font-semibold text-slate-300">Private media Q&A</p>
        ...
        {messages.map((m, i) => (
          <div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
            <div className={`max-w-[88%] rounded-lg p-4 text-sm break-words whitespace-pre-wrap shadow-sm lg:text-[15px] ${
              ...
              {m.content || (m.role === 'ai' && <span className="animate-pulse text-slate-500 font-bold tracking-widest">. . .</span>)}
            </div>

            {m.metrics && (
              <div className="mt-1 flex items-center gap-2 text-[10px] uppercase font-bold text-slate-500 pl-1 animate-fade-in">
              ...
              <div className="mt-2 flex max-w-[88%] flex-wrap gap-2">
                {m.citations.map((citation, index) => (
                  <button
                    ...
                    className="rounded-md border border-purple-500/30 bg-slate-900 px-2 py-1 font-mono text-[10px] text-purple-300 transition-colors hover:border-purple-500 hover:bg-purple-500/10"
                    title={citation.text}
                  >
      ...
      {attachments.length > 0 && (
        <div className="p-4 bg-slate-950 border-t border-slate-800 flex gap-3 overflow-x-auto shrink-0">
          {attachments.map((att, i) => (
            <div key={i} className="relative group shrink-0">
              {att.type === 'image' ? (
                <img src={att.previewUrl} className="h-16 w-16 object-cover rounded-lg border border-slate-700" alt="attachment" />
              ) : (
                <div className="h-16 w-16 bg-slate-800 rounded-lg border border-slate-700 flex flex-col items-center justify-center p-2 text-center text-slate-400">
      ...
      <div className="p-4 bg-slate-800/30 border-t border-slate-800 shrink-0">
        <div className="flex gap-2">
          <label className={`p-3 border border-slate-700 rounded-xl transition-all flex items-center justify-center group shrink-0 shadow-sm ${
            chatReady
              ? 'cursor-pointer bg-slate-800 text-slate-400 hover:bg-slate-700 hover:border-slate-600 hover:text-white'
          ...
          <input
            ...
            className="flex-1 bg-slate-950 border border-slate-700 rounded-xl px-4 py-2 text-sm text-slate-200 focus:outline-none focus:border-blue-500 disabled:cursor-not-allowed disabled:text-slate-600 transition-colors shadow-inner"
          />
          <button
            onClick={handleSend}
            disabled={busy || !chatReady}
            className="p-3 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 text-white rounded-xl hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50 transition-colors shadow-md shrink-0"
          >
