...
export const VideoMetadataItem = ({ label, value, wide = false }: VideoMetadataItemProps) => (
  <div className={wide ? 'min-w-0 sm:col-span-2' : 'min-w-0'}>
    ...
    <div className="truncate text-sm text-slate-200" title={value}>
      {value}
    </div>
  </div>
)
...
export const MediaPanelHeader = ({ isReady, isLoading, status }: MediaPanelHeaderProps) => (
    <div className="flex items-start justify-between gap-4 border-b border-slate-800 bg-slate-800/50 p-4">
    <div>
      <div className="flex items-center gap-2">
        <span className={`h-2 w-2 rounded-full ${isReady ? 'bg-green-500' : isLoading ? 'bg-yellow-400 animate-pulse' : status === 'error' ? 'bg-red-500' : 'bg-slate-500'}`} />
        <span className="text-sm font-semibold text-slate-200">Source Media</span>
      </div>
      <p className="mt-1 text-xs text-slate-500">One local video or audio file is active for analysis in this workspace.</p>
    </div>
    <span className="shrink-0 rounded-md border border-slate-700 bg-slate-950 px-2 py-1 text-[10px] font-bold uppercase tracking-widest text-slate-500">
...
export const MediaDropZone = ({ settings, onChoose, onFiles }: MediaDropZoneProps) => (
  <div
    ...
    className="flex flex-1 cursor-pointer flex-col items-center justify-center rounded-lg border border-dashed border-slate-700 bg-slate-950/60 p-8 text-center transition-colors hover:border-blue-500/70 hover:bg-blue-500/5 focus:outline-none focus:ring-2 focus:ring-blue-500/40"
  >
    <div className="mb-5 rounded-lg border border-purple-500/20 bg-purple-500/10 p-4 text-purple-300">
      <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
        <path d="m22 8-6 4 6 4V8Z" />
        <rect width="14" height="12" x="2" y="6" rx="2" ry="2" />
      </svg>
    </div>
    <p className="text-xl font-bold tracking-tight text-slate-100">Add a recording to begin</p>
    <p className="mt-3 max-w-sm text-sm leading-6 text-slate-500">
      Drop in a video or audio file, or browse from your computer. VaultClip checks the file before preparing context.
    </p>
    <div className="mt-6 rounded-lg bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-purple-500/30 transition-colors hover:opacity-90">
      Choose File
    </div>
    <div className="mt-4 rounded-lg border border-slate-800 bg-slate-900 px-3 py-2 text-[10px] font-bold uppercase tracking-wide text-slate-500">
  ...
  <div className="rounded-lg border border-purple-500/30 bg-purple-500/10 p-4">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div className="min-w-0">
        <div className="text-[10px] font-bold uppercase tracking-wide text-purple-300">{statusLabel}</div>
        <p className="mt-1 text-base font-semibold text-slate-100">{title}</p>
        <p className="mt-1 text-sm leading-6 text-purple-100/70">{detail}</p>
      </div>
        ...
        <div className="flex shrink-0 flex-col gap-2 sm:min-w-40">
          ...
          {secondaryAction && (
            <button
              ...
              className="rounded-lg border border-slate-700 px-4 py-2 text-xs font-bold uppercase tracking-wide text-slate-300 transition-colors hover:border-slate-600 hover:bg-slate-800 hover:text-white disabled:cursor-not-allowed disabled:border-slate-800 disabled:text-slate-600"
            >
...
export const MediaPreview = ({ mediaKind, fileUrl }: MediaPreviewProps) => {
  ...
  return mediaKind === 'audio' ? (
    <div className="rounded-lg border border-slate-800 bg-slate-950/70 p-5">
      <p className="mb-3 text-sm font-semibold text-slate-100">Audio preview</p>
    ...
    <video
      ...
      controls
      className="aspect-video min-h-[360px] w-full rounded-lg border border-slate-800 bg-black object-contain lg:min-h-[460px]"
    />
  )
}
...
export const MediaMetadataGrid = ({ mediaKind, name, size, duration, type, sessionId }: MediaMetadataGridProps) => (
  <div className="grid gap-4 rounded-lg border border-slate-800 bg-slate-950/70 p-5 sm:grid-cols-2 xl:grid-cols-3">
...
export const AudioSection = ({
  ...
  return (
    <div className="rounded-lg border border-slate-800 bg-slate-950/70 p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-sm font-semibold text-slate-100">Transcription audio</p>
        ...
        {isVideoInput && (
          <button
            ...
            className="shrink-0 rounded-lg border border-blue-500/50 bg-blue-600 px-4 py-2 text-xs font-bold uppercase tracking-wide text-white transition-colors hover:bg-blue-500 disabled:cursor-not-allowed disabled:border-slate-700 disabled:bg-slate-800 disabled:text-slate-500"
          >
    ...
    {segments.length > 0 && (
      <div className="mt-4 max-h-72 space-y-3 overflow-y-auto pr-1">
        {segments.map((segment) => (
          <div key={segment.id} className="rounded-lg border border-slate-800 bg-slate-900/70 p-3">
...
export const PanelActions = ({ onReplace, onReset }: PanelActionsProps) => (
  <div className="flex gap-3">
    <button
      type="button"
      onClick={onReplace}
      className="flex-1 rounded-lg border border-purple-500/50 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 px-4 py-3 text-sm font-bold text-white transition-colors hover:opacity-90"
    >
      Replace File
    </button>
    <button
      type="button"
      onClick={onReset}
      className="rounded-lg border border-slate-700 px-4 py-3 text-sm font-bold text-slate-300 transition-colors hover:border-slate-600 hover:bg-slate-800 hover:text-white"
    >
      Reset
    </button>
  </div>
